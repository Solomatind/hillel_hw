...
public class FilmLibraryQuery {
    ...
    public final static String CURRENT_PREV_YEAR_FILMS = "SELECT films.*, directors.*, GROUP_CONCAT(actors.actor_name SEPARATOR ', ') AS actor_names, GROUP_CONCAT(actors.actor_birthday SEPARATOR ', ') AS actor_birthdays FROM films JOIN directors ON films.director_id = directors.iddirectors JOIN films_actors ON films.idfilms = films_actors.idfilms JOIN actors ON films_actors.idactors = actors.idactors WHERE films.release_date = 2019 OR films.release_date = 2020 GROUP BY films.idfilms;";
